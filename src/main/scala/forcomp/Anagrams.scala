...
object Anagrams {
  ...
  /** Converts the word into its character occurrence list.
    ...
    */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase().groupBy(g => g).mapValues(x => x.length).toList.sortWith(_._1 < _._1)
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    ...
    if (occurrences.isEmpty) List(List())
      ...
      combinations(occurrences.tail) flatMap (combined => 0 to n map (x => makeOccur(c, x) ++ combined))
    }.distinct
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    def count(char: Char, occurrences: Occurrences): Int = {
      val elem = occurrences.find(_._1 == char)
      if (elem.isEmpty) 0 else elem.get._2
    }

    x.map(x => (x._1, x._2 - count(x._1, y))).filter(a => a._2 > 0).sortWith(_._1 < _._1)
  }
      ...
      def occurrenceAnagrams(occurrences: Occurrences): List[Sentence] =
        if (occurrences.isEmpty) List(List())
          ...
          combinations(occurrences)
            .flatMap(x => dictionaryByOccurrences(x)
              .flatMap(y => occurrenceAnagrams(subtract(occurrences, x))
                .map(z => y :: z)))
